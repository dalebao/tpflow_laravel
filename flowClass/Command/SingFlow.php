...
class SingFlow
{
    ...
    {
        $sing_id = DB::connection('workflow')->table('run')->where('id', $config['run_id'])->value('sing_id');
    ...
    {
        return DB::connection('workflow')->table('run_sign')->where('id', $sing_sign)->update(['is_agree' => 1, 'content' => $check_con, 'dateline' => time()]);
    }
    ...
    {
        return DB::connection('workflow')->table('run')->where('id', $run_id)->update(['is_sing' => 0]);
    }
    ...
    {
        return DB::connection('workflow')->table('run')->where('id', $run_id)->update(['run_flow_process' => $run_process]);
    }
    ...
    public function AddSing($config)
    {
        ...
        $run_sign = DB::connection('workflow')->table('run_sign')->insertGetId($data);
        if (!$run_sign) {
            return false;
        }
        return $run_sign;
    ...
    {
        return DB::connection('workflow')->table('run')->where('id', $run_id)->update(['is_sing' => 1, 'sing_id' => $sid, 'endtime' => time()]);
    }
